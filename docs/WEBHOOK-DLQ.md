...
- All admin endpoints require an admin JWT role.
- `webhookSecret` is **never** returned in API responses.
- `flushToDLQ()` implementations must redact `webhookSecret` before persisting
  (enforced by `WebhookDLQStorage` — the `webhook_secret` column is excluded
  from all list/get responses).
- Replay requires a `reason` field (min 5 chars) for audit trail.
- Deduplication prevents a replayed entry from being written twice.
